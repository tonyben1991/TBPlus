//------------------------------------------------------------------------
// 潘多拉计划 Pandora_getLowTrendLine(获取最近区间内的低点构成的趋势线)
// 版本 20160810
// 测试品种 通用
// 周期 通用(默认15分钟周期)
    ...
    if(GetNumericArraySize(arr_lines) > 0 && GetNumericArraySize(arr_points) > 0)
    {
        for i = 0 to GetNumericArraySize(arr_lines) / 2 - 1
        {
            k = (Min(o[CurrentBar - arr_lines[i * 2 + 1]], c[CurrentBar - arr_lines[i * 2 + 1]])
                - Min(o[CurrentBar - arr_lines[i * 2]], c[CurrentBar - arr_lines[i * 2]])) / proportionYX / (arr_lines[i * 2 + 1] - arr_lines[i * 2]); //k = (y2 - y1) / (x2 - x1)
