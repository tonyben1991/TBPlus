    ...
    if(GetNumericArraySize(arr_lines) > 0 && GetNumericArraySize(arr_points) > 0)
    {
        ...
        for i = 0 to GetNumericArraySize(arr_lines) / 2 - 1
        {
            k = (Max(o[CurrentBar - arr_lines[i * 2 + 1]], c[CurrentBar - arr_lines[i * 2 + 1]])
                - Max(o[CurrentBar - arr_lines[i * 2]], c[CurrentBar - arr_lines[i * 2]])) / (arr_lines[i * 2 + 1] - arr_lines[i * 2]);    //k = (y2 - y1) / (x2 - x1)
            ...
            for j = 0 to GetNumericArraySize(arr_points) - 1
            {
                ...
                {
                    if(Max(o[CurrentBar - arr_points[j]], c[CurrentBar - arr_points[j]]) >= b + k * arr_points[j])
                    {    //因为是高点的趋势线，所以判断是否在直线的下方，依据y = b + kx，y3 < y则在线下方，否则只要有一个点不满足is_one_side条件都过滤掉
                        is_one_side = false;
                        break;
                    }
                }
